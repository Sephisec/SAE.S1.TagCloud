...
//Pour le tri décroissant du dictionnaire par valeur dans exportResults()
using System.Linq;
...
class nuageMots{
	...
	public static void multipleFiles(string[] paths){
		/*	multipleFiles:	proc
		*	Execute l'ensemble des opérations nécessaires
		*	pour construire un dictionnaire d'occurence
		*	par fichier, pour tous les fichiers du tableau passés en paramètre.
		*	Appelle la fonction d'export des résultats
		*	param:	paths:			string[]:									tableau des chemins d'accès des fichiers à analyser
		*	local:	path:			string:										string temporaire pour le parcours
		*			filename:		string:										nom du fichier de base (sans le chemin d'accès)
		*			words:			string[]:									mots du fichier
		*			wordRoots:		List<List<string>>:							mots du fichier avant et après la suppression des terminaisons
		*
		*			occDetailed:	Dictionary<string,Dictionary<string,int>>:	pour chaque racine on a un dictionnaire de mots qui ont
		*																		conduit à cette racine et leurs nombres d'occurence
		*
		*			occFinal:		Dictionary<string,int>:						dictionnaire final (mot-nbOccurence)
		*/
		foreach(string path in paths){
			...
			exportResults(occFinal, filename);
		}
	}

	public static void exportResults(Dictionary<string,int> Xdict, string filename){
		/*	exportResults:	proc
		*	Exporte le dictionnaire d'occurences final dans le dossier
		*	de résultats sous la forme "./results/OUTfilename".
		*	Si un fichier du même nom existe déjà, son contenu
		*	est écrasé
		*	param:	Xdict:		Dictionary<string,int>:		dictionnaire final
		*			filename:	string:						nom du fichier (nom.txt)
		*	local:	sw:			StreamWriter:				Permet d'écrire dans le .txt
		*			kvp:		KeyValuePair<string,int>:	Paire clef-valeur pour le parcours
		*/
		StreamWriter sw = File.CreateText("./results/OUT"+filename);
	...
	public static string wordModify(string baseword, string terminaison, string replace, int m){
		...
		*/
		...
		if(replace=="epsilon"){
			tempWord=baseword.Substring(0, wordLen-endingLen);
